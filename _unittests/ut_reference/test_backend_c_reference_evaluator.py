...
class CReferenceEvaluatorBackend(onnx.backend.base.Backend):
    ...
    def create_inference_session(cls, model):
        return CReferenceEvaluator(model)

    @classmethod
    def prepare(
        cls, model: Any, device: str = "CPU", **kwargs: Any
    ) -> CReferenceEvaluatorBackendRep:
        # if isinstance(model, CReferenceEvaluatorBackendRep):
        #    return model
        if isinstance(model, CReferenceEvaluator):
...
backend_test = onnx.backend.test.BackendTest(CReferenceEvaluatorBackend, __name__)
...
if sys.platform == "darwin":
    backend_test.exclude("test_basic_conv")
