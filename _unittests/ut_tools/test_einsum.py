...
class TestEinsum(ExtTestCase):
    ...
    def test_case_2_A(self):
        for strat in ["numpy", "simple"]:
            with self.subTest(strategy=strat):
                self.common_test_case_2("abc,cd->abc", strategy=strat, verbose=False)
        ...
        for i, (eq, exp) in enumerate(res):
            with self.subTest(equation=eq, index=i, total=len(res)):
                ...
                if verbose:
                    print("\n########################################clean=False")
                    print("#########0", eq)
                seq = decompose_einsum_equation(eq, m1.shape, m2.shape, verbose=verbose)
                res = apply_einsum_sequence(seq, m1, m2, verbose=verbose)
                self.assertEqualArray(exp, res)
        ...
        for i, (eq, exp) in enumerate(res):
            with self.subTest(equation=eq, index=i, total=len(res)):
                ...
                if verbose:
                    print("#########", eq)
                seq = decompose_einsum_equation(
                    eq, m1.shape, m2.shape, m3.shape, strategy="numpy", clean=True
                )
    ...
    def test_exc(self):
        ...
        self.assertRaise(
            lambda: EinsumSubOp(
                2, "matmul", 0, 1, axes=(0, 1), left=(0, 1), right=(0, 1)
            ),
            RuntimeError,
        )
