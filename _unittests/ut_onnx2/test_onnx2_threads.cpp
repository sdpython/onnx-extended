...
TEST(ThreadPoolTests, ParallelModelProcessing) {
  ModelProto model;
  ...
  for (int i = 0; i < num_tensors; ++i) {
    ...
    std::vector<uint8_t> values(10, static_cast<uint8_t>(i));
    tensor.add_dims(1);
    tensor.add_dims(10);
  ...
  // writing
  std::string temp_filename = "test_file_write_model_proto_parralel.onnx";
  {
