...
void InternalOrtCpuValue::free_ort_value() {
...
OrtShape *allocate_ort_shape(size_t n) { return new OrtShape[n]; }

InternalOrtCpuValue *allocate_ort_cpu_value(size_t n) {
  return new InternalOrtCpuValue[n];
}

void delete_ort_shape(OrtShape *p) { delete[] p; }

void delete_internal_ort_cpu_value(InternalOrtCpuValue *p) { delete[] p; }
...
public:
  ...
  size_t Run(size_t n_inputs, OrtShape *shapes, InternalOrtCpuValue *values,
    ...
    ONNXTensorElementDataType elem_type;
    *out_values = new InternalOrtCpuValue[n_outputs_];
    *out_shapes = new OrtShape[n_outputs_];
    size_t size, n_dims;
    void *data;
