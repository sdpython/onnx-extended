...
class LargeModelContainer:
    ...
    def set_large_initializers(self, large_initializers: Dict[str, np.ndarray]):
        ...
        for k in large_initializers:
            if not k.startswith("#"):
                raise ValueError(
                    f"The location {k!r} must start with '#' "
                    f"to be ignored by check model."
                )
    ...
    def _save_lonnx(self, file_path: str):
        ...
        with open(file_path, "wb") as f:
            ...
            for location, np_tensor in self.large_initializers.items():
                ...
                if graph_index == -1:
                    raise RuntimeError(
                        f"Unable to find tensor location {location!r}."
                        f"Did you change the structure?"
                    )
                if found_tensor is None:
                    raise RuntimeError(
                        f"Large initializer with location={location!r} "
                        f"cannot be found in the model."
                    )
                init = found_tensor
                ...
                if len(buffer) != size:
                    raise RuntimeError(
                        ...
                        f"is expected to have size {size} "
                        f"but larger tensor has size {len(buffer)}."
                    )
    ...
    def save(
        ...
        :param file_format: format to use, it not specified, the extension is used
            to determine the format, `.onnx` means every
            tensor is saved as a separate file
