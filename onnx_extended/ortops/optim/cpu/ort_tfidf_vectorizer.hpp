...
TfIdfVectorizerKernel<TIN, TOUT>::TfIdfVectorizerKernel(
  ...
  std::vector<int64_t> ngram_counts = KernelInfoGetOptionalAttribute(
      api, info, "ngram_counts", std::vector<int64_t>());
  std::vector<int64_t> ngram_indexes = KernelInfoGetOptionalAttribute(
      api, info, "ngram_indexes", std::vector<int64_t>());
  std::vector<int64_t> pool_int64s = KernelInfoGetOptionalAttribute(
      api, info, "pool_int64s", std::vector<int64_t>());

  std::string pool_strings =
      KernelInfoGetOptionalAttributeString(api, info, "pool_strings", "");
  EXT_ENFORCE(pool_strings == "",
              "pool_strings must be empty, use pool_int64s instead.");

  std::vector<float> weights = KernelInfoGetOptionalAttribute(
      api, info, "weights", std::vector<float>());
