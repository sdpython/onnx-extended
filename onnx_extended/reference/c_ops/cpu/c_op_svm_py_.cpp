...
namespace onnx_c_ops {
	...
	public:
		...
		void init(py_array_ntype coefficients, py_array_ntype kernel_params,
			...
			std::vector<int64_t> emptyi;
				RuntimeSVMCommon<NTYPE>::init(vcoefficients, vkernel_params, kernel_type, post_transform, vrho,
				vsupport_vectors, n_supports, one_class, 
				empty, empty, emptyi, emptyi);
	...
	private:
		void compute_gil_free(const std::vector<int64_t>& x_dims, int64_t N,
			int64_t stride, const py_array_ntype& X, py_array_int64& Y, py_array_ntype& Z, int64_t n_columns) const {
			RuntimeSVMCommon<NTYPE>::compute_classifier(
				x_dims, N, stride, (const NTYPE*)X.data(), (int64_t*)Y.data(), (NTYPE*)Z.data());
		}
		};
