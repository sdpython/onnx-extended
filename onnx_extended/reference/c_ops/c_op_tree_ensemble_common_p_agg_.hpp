...
template <typename NTYPE> struct SparseValue {
  int64_t i;
  NTYPE value;
};

enum MissingTrack { NONE, TRUE, FALSE };

template <typename NTYPE> struct TreeNodeElement {
  ...
  inline bool is_not_leaf() const { return truenode != nullptr; }
  int64_t get_sizeof() {
    return sizeof(TreeNodeElement) +
           weights_vect.size() * sizeof(SparseValue<NTYPE>);
  }
};
...
template <typename NTYPE> struct ArrayTreeNodeElement {
  ...
  int64_t get_sizeof() {
    int64_t res =
        sizeof(ArrayTreeNodeElement<NTYPE>) +
        id.size() * sizeof(TreeNodeElementId) +
        feature_id.size() * sizeof(size_t) + value.size() * sizeof(NTYPE) +
        hitrates.size() * sizeof(NTYPE) + mode.size() * sizeof(NODE_MODE) +
        truenode.size() * sizeof(size_t) + falsenode.size() * sizeof(truenode) +
...
template <typename NTYPE> class _Aggregator {
protected:
  size_t n_trees_;
...
public:
  inline _Aggregator(size_t n_trees, const int64_t &n_targets_or_classes,
  ...
  inline void ProcessTreeNodePrediction1(
      NTYPE *predictions, const ArrayTreeNodeElement<NTYPE> &array_nodes,
      size_t node_id, unsigned char *has_predictions) const {}

  inline void MergePrediction1(NTYPE *predictions,
                               unsigned char *has_predictions,
                               NTYPE *predictions2,
                               unsigned char *has_predictions2) const {}

  inline size_t FinalizeScores1(NTYPE *Z, NTYPE &val, unsigned char &has_scores,
                                int64_t *Y = 0) const {
    val = has_scores ? (val + origin_) : origin_;
    *Z = post_transform_ == POST_EVAL_TRANSFORM::PROBIT ? ComputeProbit(val)
                                                        : val;
    return 1;
  }
  ...
  void ProcessTreeNodePrediction(NTYPE *predictions,
                                 TreeNodeElement<NTYPE> *root,
                                 unsigned char *has_predictions) const {}
  ...
  void MergePrediction(int64_t n, NTYPE *predictions,
                       unsigned char *has_predictions, NTYPE *predictions2,
                       unsigned char *has_predictions2) const {}

  size_t FinalizeScores(NTYPE *scores, unsigned char *has_scores, NTYPE *Z,
    ...
    return write_scores(this->n_targets_or_classes_, scores, post_transform_, Z,
                        add_second_class);
  }
};
...
template <typename NTYPE> class _AggregatorSum : public _Aggregator<NTYPE> {
...
public:
  inline _AggregatorSum<NTYPE>(size_t n_trees,
                               ...
                               POST_EVAL_TRANSFORM post_transform,
                               const std::vector<NTYPE> *base_values)
      : _Aggregator<NTYPE>(n_trees, n_targets_or_classes, post_transform,
                           base_values) {}
  ...
  inline void MergePrediction1(NTYPE *predictions,
                               unsigned char *has_predictions,
                               const NTYPE *predictions2,
                               const unsigned char *has_predictions2) const {
  ...
  void ProcessTreeNodePrediction(NTYPE *predictions,
                                 TreeNodeElement<NTYPE> *root,
                                 unsigned char *has_predictions) const {
    for (auto it = root->weights_vect.cbegin(); it != root->weights_vect.cend();
         ++it) {
  ...
  void ProcessTreeNodePrediction(NTYPE *predictions,
                                 ...
                                 size_t node_id,
                                 unsigned char *has_predictions) const {
    for (auto it = array_nodes.weights[node_id].cbegin();
         it != array_nodes.weights[node_id].cend(); ++it) {
      predictions[it->i] += it->value;
      has_predictions[it->i] = 1;
    }
  }
  ...
  size_t FinalizeScores(NTYPE *scores, unsigned char *has_scores, NTYPE *Z,
    ...
    return write_scores(this->n_targets_or_classes_, scores,
                        this->post_transform_, Z, add_second_class);
  }
};
...
public:
  inline _AggregatorAverage<NTYPE>(size_t n_trees,
                                   ...
                                   POST_EVAL_TRANSFORM post_transform,
                                   const std::vector<NTYPE> *base_values)
      : _AggregatorSum<NTYPE>(n_trees, n_targets_or_classes, post_transform,
                              base_values) {}
...
template <typename NTYPE> class _AggregatorMin : public _Aggregator<NTYPE> {
public:
  ...
  inline void ProcessTreeNodePrediction1(
    ...
    *predictions =
        (!(*has_predictions) || val < *predictions) ? val : *predictions;
    *has_predictions = 1;
  }
  ...
  void ProcessTreeNodePrediction(NTYPE *predictions,
    ...
    for (auto it = root->weights_vect.cbegin(); it != root->weights_vect.cend();
         ++it) {
      predictions[it->i] =
          (!has_predictions[it->i] || it->value < predictions[it->i])
              ? it->value
              : predictions[it->i];
      has_predictions[it->i] = 1;
    }
  ...
  void MergePrediction(int64_t n, NTYPE *predictions,
    ...
    for (int64_t i = 0; i < n; ++i) {
      if (has_predictions2[i]) {
        predictions[i] =
            has_predictions[i] && (predictions[i] < predictions2[i])
                ? predictions[i]
                : predictions2[i];
        has_predictions[i] = 1;
      }
...
template <typename NTYPE> class _AggregatorMax : public _Aggregator<NTYPE> {
public:
...
public:
  ...
  void get_max_weight(const NTYPE *classes, const unsigned char *has_scores,
  ...
  inline size_t FinalizeScores1(NTYPE *Z, NTYPE &val, unsigned char &has_score,
                                int64_t *Y = 0) const {
    NTYPE scores[2] = {0, 0};
    ...
    *Y = _set_score_binary(write_additional_scores, &(scores[0]), has_scores);
    return write_additional_scores == -1
               ? write_scores(this->n_targets_or_classes_, scores,
                              this->post_transform_, Z, write_additional_scores)
  ...
  size_t FinalizeScores(NTYPE *scores, unsigned char *has_scores, NTYPE *Z,
    ...
    if (this->n_targets_or_classes_ > 2) {
      // add base values
      for (int64_t k = 0,
                   end = static_cast<int64_t>(this->base_values_->size());
           k < end; ++k) {
        if (!has_scores[k]) {
          has_scores[k] = true;
      ...
      *Y = _set_score_binary(write_additional_scores, &(scores[0]),
                             &(has_scores[0]));
    }

    return write_scores(n_classes, scores, this->post_transform_, Z,
                        write_additional_scores);
  }
};
