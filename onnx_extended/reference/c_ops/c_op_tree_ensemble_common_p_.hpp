 ...
 */
template <typename NTYPE> class RuntimeTreeEnsembleCommonP {
public:
...
public:
  ...
  void init_c(const std::string &aggregate_function,
  ...
  TreeNodeElement<NTYPE> *ProcessTreeNodeLeave(TreeNodeElement<NTYPE> *root,
                                               const NTYPE *x_data) const;
  ...
  py::array_t<NTYPE> compute_tree_outputs_agg(
      py::array_t<NTYPE, py::array::c_style | py::array::forcecast> X,
      const AGG &agg) const;

  py::array_t<int> debug_threshold(
  ...
  template <typename AGG>
  py::array_t<NTYPE>
  compute_agg(py::array_t<NTYPE, py::array::c_style | py::array::forcecast> X,
              const AGG &agg);

  template <typename AGG>
  py::tuple compute_cl_agg(
      py::array_t<NTYPE, py::array::c_style | py::array::forcecast> X,
      const AGG &agg);
...
private:
  ...
  template <typename AGG>
  void compute_gil_free_array_structure(
      const std::vector<int64_t> &x_dims, int64_t N, int64_t stride,
      const py::array_t<NTYPE, py::array::c_style | py::array::forcecast> &X,
      py::array_t<NTYPE, py::array::c_style | py::array::forcecast> &Z,
      py::array_t<int64_t, py::array::c_style | py::array::forcecast> *Y,
...
void RuntimeTreeEnsembleCommonP<NTYPE>::init_c(
  ...
  for (i = 0; i < nodes_treeids.size(); ++i) {
    ...
    node->missing_tracks =
        i < (size_t)nodes_missing_value_tracks_true.size()
            ? (nodes_missing_value_tracks_true[i] == 1 ? MissingTrack::TRUE
                                                       : MissingTrack::FALSE)
            : MissingTrack::NONE;
    ...
    idi.insert(
        std::pair<TreeNodeElementId, TreeNodeElement<NTYPE> *>(node->id, node));
  ...
  for (i = 0; i < (size_t)n_nodes_; ++i) {
    ...
    if (coor.node_id >= 0 && coor.node_id < n_nodes_) {
      ...
      if ((it->truenode->id.tree_id != it->id.tree_id) ||
        ...
        char buffer[1000];
        sprintf(buffer,
                "truenode [%d] is pointing either to itself [node id=%d], "
                "either to another tree [%d!=%d-%d].",
    ...
    if (coor.node_id >= 0 && coor.node_id < n_nodes_) {
      ...
      if ((it->falsenode->id.tree_id != it->id.tree_id) ||
        ...
        throw std::invalid_argument("One falsenode is pointing either to "
                                    "itself, either to another tree.");
        char buffer[1000];
        sprintf(buffer,
                "falsenode [%d] is pointing either to itself [node id=%d], "
                "either to another tree [%d!=%d-%d].",
                (int)i, (int)it->id.node_id, (int)it->id.tree_id,
  ...
  for (i = 0; i < target_class_nodeids.size(); i++) {
    ...
    if (idi.find(ind) == idi.end()) {
      char buffer[1000];
      sprintf(buffer, "Unable to find node %d-%d (weights).", (int)coor.tree_id,
              (int)coor.node_id);
      throw std::invalid_argument(buffer);
    }
  ...
  if (array_structure_)
    ...
    throw std::invalid_argument(
        "array_structure must be enabled for para_tree.");
}
...
void RuntimeTreeEnsembleCommonP<NTYPE>::switch_to_array_structure() {
  ...
  for (int64_t i = 0; i < n_nodes_; ++i) {
    ...
    array_nodes_.truenode[i] = nodes_[i].truenode == nullptr
                                   ? ID_LEAF_TRUE_NODE
                                   : std::distance(first, nodes_[i].truenode);
    array_nodes_.falsenode[i] = nodes_[i].falsenode == nullptr
                                    ? ID_LEAF_TRUE_NODE
                                    : std::distance(first, nodes_[i].falsenode);
  ...
  for (int64_t i = 0; i < n_nodes_; ++i) {
    if (nodes_[i].is_not_leaf() != array_nodes_.is_not_leaf(i))
      throw std::invalid_argument(
          MakeString("Inconsistent results for is_node_leaf ", i,
                     " true: ", array_nodes_.truenode[i],
                     " false:", array_nodes_.falsenode[i],
...
RuntimeTreeEnsembleCommonP<NTYPE>::get_nodes_modes() const {
...
py::array_t<NTYPE> RuntimeTreeEnsembleCommonP<NTYPE>::compute_agg(
  ...
  py::array_t<NTYPE, py::array::c_style | py::array::forcecast> Z(
      x_dims[0] * n_targets_or_classes_);
...
py::detail::unchecked_mutable_reference<int64_t, 1> _mutable_unchecked1(
    py::array_t<int64_t, py::array::c_style | py::array::forcecast> &Z) {
  return Z.mutable_unchecked<1>();
}
...
void RuntimeTreeEnsembleCommonP<NTYPE>::compute_gil_free(
  ...
  if (n_targets_or_classes_ == 1) {
    if ((N == 1) && (n_trees_ <= omp_tree_)) {
      DEBUGPRINT("A")
      NTYPE scores = 0;
      unsigned char has_scores = 0;
      for (int64_t j = 0; j < n_trees_; ++j)
        agg.ProcessTreeNodePrediction1(
            &scores, ProcessTreeNodeLeave(roots_[j], x_data), &has_scores);

      agg.FinalizeScores1(
          (NTYPE *)Z_.data(0), scores, has_scores,
      ...
      for (int64_t j = 0; j < n_trees_; ++j) {
        agg.ProcessTreeNodePrediction1(&(local_scores[j]),
      ...
      agg.FinalizeScores1(
          (NTYPE *)Z_.data(0), scores, has_scores,
          Y == nullptr ? nullptr : (int64_t *)_mutable_unchecked1(*Y).data(0));
    } else if ((N <= omp_N_) && (n_trees_ <= omp_tree_)) {
      DEBUGPRINT("C")
      NTYPE scores;
      ...
      for (int64_t i = 0; i < N; ++i) {
        ...
        for (j = 0; j < (size_t)n_trees_; ++j)
          agg.ProcessTreeNodePrediction1(
              &scores, ProcessTreeNodeLeave(roots_[j], x_data + i * stride),
              &has_scores);
        agg.FinalizeScores1((NTYPE *)Z_.data(i), scores, has_scores,
                            Y == nullptr
                                ? nullptr
                                : (int64_t *)_mutable_unchecked1(*Y).data(i));
      }
    } else {
      DEBUGPRINT("D")
    ...
    */
    ...
    if ((N == 1) && (n_trees_ <= omp_tree_)) {
      ...
      for (int64_t j = 0; j < n_trees_; ++j) {
        agg.ProcessTreeNodePrediction(scores.data(),
                                      ProcessTreeNodeLeave(roots_[j], x_data),
                                      has_scores.data());
      }
      agg.FinalizeScores(
          scores.data(), has_scores.data(), (NTYPE *)Z_.data(0), -1,
          Y == nullptr ? nullptr : (int64_t *)_mutable_unchecked1(*Y).data(0));
    } else if (N == 1) {
      DEBUGPRINT("F")
      ...
      for (size_t th = 1; th < (size_t)nth; ++th) {
        agg.MergePrediction(n_targets_or_classes_, scores.data(),
                            has_scores.data(),
      ...
      for (int64_t i = 0; i < N; ++i) {
        ...
        for (size_t j = 0; j < roots_.size(); ++j)
          agg.ProcessTreeNodePrediction(
              p_score, ProcessTreeNodeLeave(roots_[j], local_x_data),
              p_has_score);
  ...
  if (n_targets_or_classes_ == 1) {
    if ((N == 1) && ((omp_get_max_threads() <= 1) || (n_trees_ <= omp_tree_))) {
      ...
      for (int64_t j = 0; j < n_trees_; ++j)
        agg.ProcessTreeNodePrediction1(
            &scores, array_nodes_,
            ProcessTreeNodeLeave(array_nodes_.root_id[j], x_data), &has_scores);
      ...
      agg.FinalizeScores1(
          (NTYPE *)Z_.data(0), scores, has_scores,
          Y == nullptr ? nullptr : (int64_t *)_mutable_unchecked1(*Y).data(0));
    } else if ((nth > 1) && para_tree_ && (n_trees_ > omp_tree_)) {
      DEBUGPRINT("O")
      std::vector<NTYPE> local_scores(omp_tree_N_ * nth);
      ...
      for (int64_t batch = 0; batch < N; batch += omp_tree_N_) {
        ...
        for (int64_t j = 0; j < n_trees_; ++j) {
          ...
          for (int64_t i = batch; i < batch_end;
               ++i, local_x_data += stride, ++p_score, ++p_has_score) {
            agg.ProcessTreeNodePrediction1(
        ...
        for (int64_t i = batch; i < batch_end; ++i) {
          ...
          for (int64_t j = 1; j < nth;
               ++j, pp_score += omp_tree_N_, pp_has_score += omp_tree_N_)
      ...
      for (int64_t i = 0; i < N; ++i) {
        ...
        for (j = 0; j < (size_t)n_trees_; ++j)
          agg.ProcessTreeNodePrediction1(
              &scores, array_nodes_,
              ProcessTreeNodeLeave(array_nodes_.root_id[j],
                                   x_data + i * stride),
              &has_scores);
        agg.FinalizeScores1((NTYPE *)Z_.data(i), scores, has_scores,
                            Y == nullptr
      ...
      for (int64_t i = 0; i < NB; i += omp_tree_N_) {
        ...
        for (int64_t k = 0; k < omp_tree_N_; ++k) {
          agg.FinalizeScores1(
              (NTYPE *)Z_.data(i + k), scores[k], has_scores[k],
              Y == nullptr ? nullptr
    ...
    if ((N == 1) && ((nth <= 1) || (n_trees_ <= omp_tree_))) {
      DEBUGPRINT("S")
      std::vector<NTYPE> scores(n_targets_or_classes_, (NTYPE)0);
      ...
      agg.FinalizeScores(
          scores.data(), has_scores.data(), (NTYPE *)Z_.data(0), -1,
          Y == nullptr ? nullptr : (int64_t *)_mutable_unchecked1(*Y).data(0));
    } else if (para_tree_ && (nth > 1) && (n_trees_ > omp_tree_)) {
      DEBUGPRINT("T")
      if (nth <= 0)
        throw std::invalid_argument("nth must strictly positive.");
      ...
      for (int64_t j = 0; j < n_trees_; ++j) {
        ...
        for (int64_t i = 0; i < N; ++i, local_x_data += stride,
                     p_score += n_targets_or_classes_,
                     p_has_score += n_targets_or_classes_) {
          agg.ProcessTreeNodePrediction(
              p_score, array_nodes_, ProcessTreeNodeLeave(node, local_x_data),
              p_has_score);
        }
      }
      ...
      for (int64_t i = 0; i < N; ++i) {
        ...
        unsigned char *p_has_score =
            &local_has_scores[i * n_targets_or_classes_];
        NTYPE *pp_score = p_score + size_obs;
        unsigned char *pp_has_score = p_has_score + size_obs;
        for (size_t j = 1; j < (size_t)nth;
             ++j, pp_score += size_obs, pp_has_score += size_obs) {
          agg.MergePrediction(n_targets_or_classes_, p_score, p_has_score,
                              pp_score, pp_has_score);
        }
        agg.FinalizeScores(p_score, p_has_score,
                           (NTYPE *)Z_.data(i * n_targets_or_classes_), -1,
                           Y == nullptr
...
template <typename NTYPE>
TreeNodeElement<NTYPE> *RuntimeTreeEnsembleCommonP<NTYPE>::ProcessTreeNodeLeave(
  ...
  if (same_mode_) {
    ...
    while (root->is_not_leaf()) {
      ...
      default: {
        std::ostringstream err_msg;
        err_msg << "Invalid mode of value: "
                << static_cast<std::underlying_type<NODE_MODE>::type>(
                       root->mode);
        throw std::invalid_argument(err_msg.str());
      }
      }
...
#define TREE_FIND_VALUE_ID(CMP)                                                \
  if (has_missing_tracks_) {                                                   \
    ...
    while (array_nodes_.is_not_leaf(root_id)) {                                \
      ...
      root_id =                                                                \
          (val CMP array_nodes_.value[root_id] ||                              \
    ...
    while (array_nodes_.is_not_leaf(root_id)) {                                \
      ASSERTTREE((root_id >= 0) && ((int64_t)root_id < n_nodes_), "root_id")   \
      root_id = x_data[array_nodes_.feature_id[root_id]] CMP array_nodes_      \
                        .value[root_id]                                        \
                    ? array_nodes_.truenode[root_id]                           \
                    : array_nodes_.falsenode[root_id];                         \
    }                                                                          \
...
size_t RuntimeTreeEnsembleCommonP<NTYPE>::ProcessTreeNodeLeave(
  ...
  if (same_mode_) {
    ...
    case NODE_MODE::BRANCH_LEQ:
      if (has_missing_tracks_) {
        ...
        while (array_nodes_.is_not_leaf(root_id)) {
          ...
          val = x_data[array_nodes_.feature_id[root_id]];
          root_id =
              (val <= array_nodes_.value[root_id] ||
               (array_nodes_.is_missing_track_true[root_id] && _isnan_(val)))
                  ? array_nodes_.truenode[root_id]
                  : array_nodes_.falsenode[root_id];
        }
        ...
        while (array_nodes_.is_not_leaf(root_id)) {
          ASSERTTREE((root_id >= 0) && ((int64_t)root_id < n_nodes_), "root_id")
          root_id = x_data[array_nodes_.feature_id[root_id]] <=
                            array_nodes_.value[root_id]
                        ? array_nodes_.truenode[root_id]
                        : array_nodes_.falsenode[root_id];
        }
      }
    ...
    while (array_nodes_.is_not_leaf(root_id)) {
      ...
      case NODE_MODE::BRANCH_LEQ:
        root_id =
            val <= threshold || (array_nodes_.is_missing_track_true[root_id] &&
                                 _isnan_(val))
                ? array_nodes_.truenode[root_id]
                : array_nodes_.falsenode[root_id];
        break;
...
py::array_t<int> RuntimeTreeEnsembleCommonP<NTYPE>::debug_threshold(
  ...
  if (array_structure_)
    throw std::invalid_argument(
        "debug_threshold not implemented if array_structure is true.");
  ...
  std::vector<ssize_t> strides = {(ssize_t)(values.size() * sizeof(int)),
                                  (ssize_t)sizeof(int)};
  return py::array_t<NTYPE>(py::buffer_info(
      &result[0], sizeof(NTYPE), py::format_descriptor<NTYPE>::format(), 2,
...
py::array_t<NTYPE> RuntimeTreeEnsembleCommonP<NTYPE>::compute_tree_outputs_agg(
  ...
  if (array_structure_)
    throw std::invalid_argument(
        "compute_tree_outputs_agg not implemented if array_structure is true.");
